            ...
            <div className='grid sm:grid-cols-3 grid-cols-1 gap-4'>
                {projects.map(project => (
                    <div key={project.id} className={`
                        group/item sm:h-[auto] h-[18rem] relative flex flex-col sm:pt-0 m-2 justify-center 
                        items-center skillsborder border-y-[3px]
                        ${first.includes(project.id) && 'sm:rounded-s-[25px] sm:rounded-[0] rounded-[25px]'}
                        ${middle.includes(project.id) && 'sm:rounded-[0px] rounded-[25px]'}
                        ${last.includes(project.id) && 'sm:rounded-e-[25px] sm:rounded-[0] rounded-[25px]'}
                    `}
                    >
                        ...
                        <h3 className='absolute top-0 sm:hidden block font-poppins text-[1.2rem] py-1.5 w-full rounded-t-[25px] text-center'>{project.title}</h3>
                            <img src={project.img} alt={project.title} className={`sm:group-hover/item:opacity-25 transition-opacity duration-500
                                ${first.includes(project.id) && 'sm:rounded-s-[25px] sm:rounded-[0] rounded-[25px]'}
                                ${middle.includes(project.id) && 'sm:rounded-[0px] rounded-[25px]'}
                                ${last.includes(project.id) && 'sm:rounded-e-[25px] sm:rounded-[0] rounded-[25px]'}`}/>
                        <div className='absolute opacity-0 sm:group-hover/item:opacity-100 transition-opacity duration-500 text-center'>
